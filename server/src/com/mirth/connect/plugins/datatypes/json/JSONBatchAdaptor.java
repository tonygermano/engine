...
import com.mirth.connect.plugins.datatypes.json.JSONBatchProperties.SplitType;
...
public class JSONBatchAdaptor extends DebuggableBatchAdaptor {
    private Logger logger = LogManager.getLogger(this.getClass());
    private ContextFactoryController contextFactoryController = ControllerFactory.getFactory().createContextFactoryController();
    private BufferedReader bufferedReader;

    public JSONBatchAdaptor(DebuggableBatchAdaptorFactory factory, SourceConnector sourceConnector, BatchRawMessage batchRawMessage) {
    ...
    private String getMessageFromReader() throws Exception {
        JSONBatchProperties batchProperties = (JSONBatchProperties) getBatchProperties();
        ...
        if (splitType == SplitType.JavaScript) {
            ...
            try {
                ...
                MirthContextFactory contextFactory = (MirthContextFactory) getContextFactoryAndRecompile(contextFactoryController, debug, batchScriptId, batchProperties.getBatchScript());                
                
                triggerDebug(debug);

                String result = JavaScriptUtil.execute(new JavaScriptTask<String>(contextFactory, "JSON Batch Adaptor", (SourceConnector) sourceConnector) {
                    @Override
